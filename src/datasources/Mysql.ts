...
export class Mysql {
    ...
    public connect() {
        Mysql.getConnection((err, connection) => {
            ...
            connection.query("CREATE TABLE IF NOT EXISTS `users` ( `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT, `name` VARCHAR(255) NOT NULL, `customer_name` VARCHAR(255) NULL DEFAULT NULL, `customer_email` VARCHAR(255) NULL DEFAULT NULL, `ssh_ipaddress` VARCHAR(255) NULL DEFAULT NULL, `ssh_port` VARCHAR(255) NULL DEFAULT NULL, `ssh_pubkey` VARCHAR(1024) NULL DEFAULT NULL, `ssh_username` VARCHAR(255) NULL DEFAULT NULL, `local_path` VARCHAR(1024) NULL DEFAULT NULL, `remote_path` VARCHAR(1024) NULL DEFAULT NULL, PRIMARY KEY (`id`), UNIQUE INDEX `name` (`name`) ) COLLATE='latin1_swedish_ci' ENGINE=InnoDB;", (err, result) => {
                if (err) throw err;
